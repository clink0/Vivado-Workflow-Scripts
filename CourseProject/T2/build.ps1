# CourseProject T2 — Picoblaze + XADC -> binary on LEDs
# Run from repo root: .\CourseProject\T2\build.ps1
...
Write-Host "=== CourseProject T2: XADC + Picoblaze => LEDs ==="
...
$tcl = @"
create_project CP_T2 {$projDir} -part xc7a35tcpg236-1 -force
...
set_property top cp_t2_top [current_fileset]
...
$bit = "$projDir\CP_T2.runs\impl_1\cp_t2_top.bit"
if (Test-Path $bit) {
