# HW11 T3 — Pulse sensor heart rate: XADC + Picoblaze + OLED + UART waveform
# Run from repo root: .\HW11\T3\build.ps1
...
Write-Host "=== HW11 T3: Heart Rate Monitor ==="
...
if (-not (Test-Path "$TaskDir\prog.v")) {
    Write-Host "ERROR: prog.v not generated."
    exit 1
}
...
$tcl = @"
create_project T3 {$projDir} -part xc7a35tcpg236-1 -force
...
set_property top hw11_t3_top [current_fileset]
...
if {[string first "Complete" `$s] < 0} { puts "ERROR: Synthesis: `$s"; exit 1 }

reset_run impl_1
launch_runs impl_1
wait_on_run impl_1
set s [get_property STATUS [get_runs impl_1]]
if {[string first "Complete" `$s] < 0} { puts "ERROR: Impl: `$s"; exit 1 }

launch_runs impl_1 -to_step write_bitstream
wait_on_run impl_1
set s [get_property STATUS [get_runs impl_1]]
if {[string first "Complete" `$s] < 0} { puts "ERROR: Bitstream: `$s"; exit 1 }
...
$bit = "$projDir\T3.runs\impl_1\hw11_t3_top.bit"
if (Test-Path $bit) {
