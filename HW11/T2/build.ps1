# HW11 T2 — XADC + Picoblaze → binary XADC on LEDs
# Run from repo root: .\HW11\T2\build.ps1
...
Write-Host "=== HW11 T2: XADC + Picoblaze => LEDs ==="
...
# ── Step 2: Build Vivado project with XADC IP ─────────────────────────────
$vFiles = @(
    "$TaskDir\hw11_t2_top.v",
    "$TaskDir\kcpsm6.v",
    "$TaskDir\prog.v"
)
...
set_property top hw11_t2_top [current_fileset]
...
$bit = "$projDir\T2.runs\impl_1\hw11_t2_top.bit"
if (Test-Path $bit) {
